...
#ifndef _SQLWRITER_HPP_
#define _SQLWRITER_HPP_
...
public:
  ...
  void endTransaction(bool createIndex, std::string table);
  void dropTable(std::string table);
  void createTable(std::string table);
  void populateTable(std::string table, std::vector<Contour> contours);
  void createIndex(std::string table);
};

#endif // _SQLWRITER_HPP_
